...
import numpy as np
import pandas as pd
import yapftests.file_resources_test
...
def data():
    symbol = st.sidebar.selectbox("Select the symbol", ["AMAZON", "ETHUSD", "FIROOZE", "FOOLAD", "KHALIJ", "KHODRO", "TAPPICO", "TESLA"])
...
if (svrconf > lrconf):
    ...
    theMean = round(float(np.mean(svrpred)), 2)
    ...
    theMean = round(float(np.mean(lrpred)), 2)
    ...
    st.write("Dataset updated in: ETHUSD(2021-04-16), Others(2021-04-14) ")
    st.write(f"The last predicted price: {theLast} (%{theIncrease1})")
    st.write(f"The predicted mean price: {theMean} (%{theIncrease2})")
